...
- Contact form: client-side POST to Web3Forms (`https://api.web3forms.com/submit`) — needs a real
  access key dropped into the hidden `access_key` field in `index.html` (currently a placeholder)
...
1. Get a Web3Forms access key (https://web3forms.com) and replace `YOUR_WEB3FORMS_ACCESS_KEY`
   in `index.html`
2. `git init` (already done) and push to GitHub
3. Connect the GitHub repo to Vercel
4. Point DNS to Vercel once the client has transferred/configured the domain
5. Cancel Wix LAST, after the domain points to Vercel and the new site is confirmed live
